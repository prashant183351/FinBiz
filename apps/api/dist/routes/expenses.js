...
router.post('/', async (req, res) => {
    try {
        const { companyId, date, description, category, amount, gstAmount = 0, paymentMethod = 'cash', reference, notes } = req.body;
...
router.put('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { date, description, category, amount, gstAmount = 0, paymentMethod, reference, notes } = req.body;
