...
class HRService {
     ...
     */
    static async submitLeaveRequest(request, submittedBy) {
     ...
     */
    static async processLeaveRequest(leaveId, action, approvedBy, notes) {
     ...
     */
    static async getLeaveRequests(employeeId, companyId, status, limit = 50, offset = 0) {
        const where = {};
        if (employeeId)
     ...
     */
    static async addIncentive(incentive, addedBy) {
     ...
     */
    static async getIncentives(employeeId, companyId, month, year, processed) {
        const where = {};
        if (employeeId)
            where.employeeId = employeeId;
        if (companyId) {
     ...
     */
    static async processIncentivesForPayroll(employeeId, month, year, processedBy) {
     ...
     */
    static async submitComplianceRecord(compliance, submittedBy) {
     ...
     */
    static async getComplianceRecords(employeeId, companyId, type, month, year, status) {
        const where = {};
        if (employeeId)
            where.employeeId = employeeId;
        if (companyId) {
    ...
    static async getLeaveBalance(employeeId) {
        ...
        const balance = Object.keys(totalLeaves).reduce((acc, type) => {
            acc[type] = {
                ...
                remaining: totalLeaves[type] - (usedLeaves[type] || 0)
            };
            return acc;
        }, {});
        return balance;
     ...
     */
    static async generatePayslipData(employeeId, month, year) {
